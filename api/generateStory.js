  ...
  try {
    ...
    if (activityType && activityType.startsWith("peeking")) {
      ...
      prompt = `
        ...
        故事應簡潔（約100-150字），用詞生動，能讓讀者感受到當地的氛圍。
        故事必須用${language}書寫。
      `;
       if (!targetTimezone && !(city && country) && activityType === "peeking") {
            prompt = `
                你是一位富有想像力的說書人。請為一位名叫 "${userForStory}" 的角色，發揮創意，描寫一段他們此刻可能正在經歷的有趣日常片段或內心小劇場（約100-150字）。
      ...
      prompt = `
        ...
        請為一位叫做 "${userForStory}" 的角色，寫一個簡短且充滿想像力的冒險故事（大約150到200字）。
