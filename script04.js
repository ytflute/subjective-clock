...
async function displayLastRecordForCurrentUser() {
    ...
    try {
        ...
        if (!querySnapshot.empty) {
            ...
            if (lastRecord.country === "Universe" && lastRecord.city === "Unknown Planet") {
                resultTextDiv.innerHTML = `This was ${rawUserDisplayName}'s last recorded entry at <strong>${userTimeFormatted}</strong>. <br>You had already left Earth and began a new day alongside extraterrestrial beings in <strong>${finalCityName} (${finalCountryName})</strong>!`;
            } else {
                resultTextDiv.innerHTML = `This was ${rawUserDisplayName}'s last recorded entry at <strong>${userTimeFormatted}</strong>. <br>You were in sync with the people of <strong>${finalCityName} (${finalCountryName})</strong> <br>as you began a new day!`;
            }
            ...
            } else if (lastRecord.city === "Unknown Planet") {
                // 顯示宇宙訊息
                mapContainerDiv.classList.add('universe-message'); // ★ 套用宇宙樣式
                mapContainerDiv.innerHTML = "<p>In the vast universe, your location cannot be determined...</p>";
            } else {
                // 顯示「無法顯示地圖」的訊息 (此時不應有 universe-message class)
                mapContainerDiv.innerHTML = "<p>Unable to display the map. This record contains incomplete or invalid coordinate information.</p>";
            }
            // --- mapContainerDiv 處理結束 ---
            
            const recordedAtDate = lastRecord.recordedAt && lastRecord.recordedAt.toDate ? lastRecord.recordedAt.toDate().toLocaleString('zh-TW') : 'Unknown Time Record';
            ...
            debugInfoSmall.innerHTML = `(Recorded on: ${recordedAtDate})<br>(Target city latitude: ${latitudeStr}°, longtidue: ${longitudeStr}°)<br>(Target UTC offset: ${targetUTCOffsetStr}, actual city UTC offset: ${cityActualUTCOffset !== null ? cityActualUTCOffset.toFixed(2) : 'N/A'}, time zone: ${lastRecord.timezone || 'Unknown'})`;
        } else {
            // 沒有歷史記錄的情況
            resultTextDiv.innerHTML = `<p>Welcome! ${rawUserDisplayName}! There is no history recorded for this name yet.</p>`;
        ...
        resultTextDiv.innerHTML = "<p>Failed to load the latest record.</p>";
        // mapContainerDiv 和 debugInfoSmall 已經由 clearPreviousResults() 清理
    }
}
...
async function findMatchingCity() {
    ...
    if (candidateCities.length === 0) {
        resultTextDiv.innerHTML = `Today, you began your day at <strong>${userTimeFormatted}</strong> local time.<br>But sorry — it seems you’ve already left Earth and started a new day with extraterrestrial beings.`;
    ...
    if (bestMatchCity) {
        ...
        resultTextDiv.innerHTML = `Today, you<br>started a new day together with the people of <strong>${finalCityName} (${finalCountryName})</strong>!`;

        if (bestMatchCity.country_iso_code) {
            countryFlagImg.src = `https://flagcdn.com/w40/${bestMatchCity.country_iso_code.toLowerCase()}.png`;
            countryFlagImg.alt = `${finalCountryName}'s Flag`;
            countryFlagImg.style.display = 'inline-block';
        }
        ...
        if (typeof bestMatchCity.latitude === 'number' && isFinite(bestMatchCity.latitude) &&
            ...
            mapContainerDiv.innerHTML = "<p>Unable to display the map. The city's coordinate information is incomplete or invalid.</p>";
        }
        ...
        debugInfoSmall.innerHTML = `(Target city latitude: ${debugLat}°, target city latitude: ${debugTargetLat}°, longitude: ${debugMinLatDiff}°)<br>(Target UTC offset: ${debugTargetOffset}, actual city UTC offset: ${debugActualOffset}, time zone: ${bestMatchCity.timezone})`;
    ...
    async function loadHistory() {
        if (!currentDataIdentifier) { 
            historyListUl.innerHTML = '<li>Please set your display name first to view your history.</li>';
            if (historyLeafletMap) {
                historyLeafletMap.remove(); // 移除舊的地圖實例
                historyLeafletMap = null;   // 重置實例變數
            }
            historyMapContainerDiv.innerHTML = '<p>Once you've set your name, your personal history map will appear here.</p>'; 
            return;
        }
        historyListUl.innerHTML = '<li>Loading you wake journey</li>';
        // *** 修改點：只有在 Leaflet 地圖還沒建立時才設定 "載入中" ***
        if (!historyLeafletMap) { 
            historyMapContainerDiv.innerHTML = '<p>Loading your wake journey</p>';
        ...
        try {
            ...
            if (querySnapshot.empty) {
                historyListUl.innerHTML = '<li>No history recorded yet.</li>';
            ...
            querySnapshot.forEach((doc) => {
                ...
                li.innerHTML = `<span class="date">${recordDate}</span> - 
                                ...
                                Scyn at: <span class="location">${cityDisplay || 'Unknown City'}, ${countryDisplay || 'Unknown Country'}</span>`;
                ...
                } else if (record.city !== "Unknown Planet") { 
            ...
            historyMapContainerDiv.innerHTML = '<p>An error occurred while loading the history.</p>'; 
    ...
    async function loadGlobalTodayMap() {
        ...
        if (!globalLeafletMap) globalTodayMapContainerDiv.innerHTML = '<p>Loading today collective map</p>';
        else if (globalMarkerLayerGroup) globalMarkerLayerGroup.clearLayers(); 

        globalTodayDebugInfoSmall.textContent = `Search date: ${selectedDateValue}`;
        console.log(`[loadGlobalTodayMap] Search date: ${selectedDateValue}`);
